...
const SearchInput = styled.input`
  ...
  &:focus {
    ...
    box-shadow: 0 0 0 2px
      ${({ theme }) => theme.colors?.primaryLight || "rgba(124, 58, 237, 0.2)"};
  }
`;
...
const FilterButton = styled.button`
  ...
  &:hover {
    background-color: ${({ active, theme }) =>
      active
        ? theme.colors?.primaryDark || "#6025c0"
...
const CategoryTag = styled.span`
  ...
  background-color: ${({ theme }) =>
    theme.colors?.primaryLight || "rgba(124, 58, 237, 0.1)"};
  color: ${({ theme }) => theme.colors?.primary || "#7c3aed"};
  border-radius: 12px;
  font-size: 0.7rem;
  font-weight: 600;
...
const EmptyIcon = styled.div`
  font-size: 4rem;
  color: ${({ theme }) =>
    theme.colors?.primaryLight || "rgba(124, 58, 237, 0.2)"};
  margin-bottom: 1rem;
`;
...
const MyPodcasts = () => {
  ...
  useEffect(() => {
    const fetchUserPodcasts = async () => {
      try {
        ...
        const response = await axios.get("/api/podcasts/user", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        console.log("Fetched User Podcasts:", response.data.podcasts);
        setPodcasts(response.data.podcasts);
        setFilteredPodcasts(response.data.podcasts);
      } catch (error) {
        console.error(
          "Error fetching user's podcasts:",
  ...
  // Get unique genres from podcasts
  const genres = ["All", ...new Set(podcasts.map((podcast) => podcast.genre))];
    ...
    <Container>
      ...
      {podcasts.length === 0 ? (
        <EmptyState>
          ...
          <p>
            Start sharing your voice with the world by uploading your first
            podcast episode.
          </p>
          <UploadButton onClick={handleUploadClick}>
            <FaPlus /> Upload New Podcast
