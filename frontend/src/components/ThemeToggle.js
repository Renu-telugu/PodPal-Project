...
const ToggleContainer = styled.button`
  ...
  height: 32px;
  border-radius: 50px;
  border: 2px solid ${({ theme, isDarkMode }) => 
    isDarkMode ? theme.colors.secondary : theme.colors.primary};
  padding: 4px;
  overflow: hidden;
  cursor: pointer;
  box-shadow: ${({ theme, isDarkMode }) => 
    isDarkMode ? '0 0 5px rgba(108, 99, 255, 0.5)' : '0 0 5px rgba(52, 152, 219, 0.5)'};
  transition: all 0.3s ease;
  
  &:hover {
    box-shadow: ${({ theme, isDarkMode }) => 
      isDarkMode ? '0 0 8px rgba(108, 99, 255, 0.8)' : '0 0 8px rgba(52, 152, 219, 0.8)'};
  }
...
const ThemeToggle = () => {
  ...
  return (
    ...
    >
      <IconContainer>
        <Icon visible={!isDarkMode} active={!isDarkMode}>☀️</Icon>
        <Icon visible={isDarkMode} active={isDarkMode}>🌙</Icon>
      </IconContainer>
